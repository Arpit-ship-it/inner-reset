...
// ⏱️ 15-Second Cloud Transmission Timeout for Primary Attempt
const sendMessageWithTimeout = (targetId, message, timeoutMs = 15000) => {
    return Promise.race([
        whatsappClient.sendMessage(targetId, message),
        new Promise((_, reject) => setTimeout(() => reject(new Error('WA_TRANSMISSION_TIMEOUT_15S')), timeoutMs))
    ]);
};
